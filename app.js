...
(() => {
  ...
  function trueupProfile(company) {
    ...
    if(!stats?.url) return '<section class="trueup-panel"><h4>Company stats · TrueUp</h4><p>No verified TrueUp company page available yet.</p></section>';
    const value=n=>n==null?'Not published':Number(n).toLocaleString('en-US');
    const stale=stats.status==='stale';
    const checked=stats.checkedAt ? new Date(stats.checkedAt).toLocaleDateString('en-US',{month:'long',day:'numeric',year:'numeric',timeZone:'UTC'}) : 'date unavailable';
    const freshness=stats.status==='refreshed'?'Retrieved from TrueUp':stats.status==='snapshot'?'Saved TrueUp snapshot · automatic refresh unavailable':'Saved TrueUp snapshot';
    return `<section class="trueup-panel"><h4>Company stats · TrueUp</h4>${stale?'<p>A current snapshot isn’t available. Visit the company’s TrueUp page for the latest information.</p>':`<dl class="trueup-stats"><div><dt>Open roles</dt><dd>${value(stats.openJobs)}</dd></div><div><dt>Remote openings</dt><dd>${value(stats.remoteJobs)}</dd></div><div><dt>Employee growth · 12 months</dt><dd>${esc(stats.employeeGrowth || 'Not published')}</dd></div></dl><p class="trueup-method">Company-wide roles across all disciplines. Figures may be rounded. Employee growth is a Workforce.ai estimate shown by TrueUp, not a hiring trend. Missing metrics are not zero.</p>`}<a href="${esc(stats.url)}" target="_blank" rel="noopener noreferrer">View company on TrueUp <i class="ri-arrow-right-up-line" aria-hidden="true"></i></a><small>${esc(freshness)} · Checked ${esc(checked)}</small></section>`;
  }
  const companyStatsCache=new Map();
  async function refreshCompanyStats(job) {
    if(!COMPANIES[job.company]?.trueup?.url) return;
    try {
      let entry=companyStatsCache.get(job.company), pending=entry?.expires>Date.now()?entry.promise:null;
      if(!pending){pending=fetch('/api/company-stats?company='+encodeURIComponent(job.company)).then(r=>{if(!r.ok)throw Error('Stats unavailable');return r.json();});companyStatsCache.set(job.company,{promise:pending,expires:Date.now()+3600000});}
      const stats=await pending;
      if($('job-dialog').dataset.jobId!==job.id) return;
      const panel=$('job-detail').querySelector('.trueup-panel');
      if(panel)panel.outerHTML=trueupProfile({...COMPANIES[job.company],trueup:stats});
    } catch {companyStatsCache.delete(job.company);}
  }
