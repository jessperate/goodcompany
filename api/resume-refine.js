...
module.exports=async(req,res)=>{
  ...
  try{
    ...
    const ai=await fetch('https://ai-gateway.vercel.sh/v1/chat/completions',{
      ...
      body:JSON.stringify({model:'openai/gpt-5.4-mini',max_completion_tokens:5000,reasoning_effort:'low',providerOptions:{gateway:{order:['openai','azure']}},messages:[
    ...
    return res.status(200).json({text:text.trim(),model:'openai/gpt-5.4-mini'});
